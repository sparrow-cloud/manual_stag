...
> **Tip**: **루트 디렉토리**는 일반적으로 분석하려는 프로그램이 저장되어 있는 최상위 폴더인 경우가 많습니다. 다만, 분석하려는 웹 페이지가 루트 디렉토리에 위치하지 않더라도 키 파일은 항상 루트 디렉토리에 저장합니다. 예를 들어, **웹 페이지 URL**에 `http://sample.domain.com/path/index.html` 을 입력한 경우 키 파일의 위치는 `http://sample.domain.com/sparrow-cloud-key-172.txt` 입니다.
